...
import { expenses,viewAndApprove} from '../constant/tableconstant';
...
export default function CustomPaginationActionsTable(props) {
  ...
  return (
    <div>
      <Title align="center">List of Expenses</Title>
      <div className="button">
        {props.prop.location.state.type === user && 
          <AddButton {...props} />
        }
      </div>
        ...
        <TableBody>
          {post.length > 0 && post.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map(row => (
            <TableRow key={row.id}>
              ...
              {props.prop.location.state && props.prop.location.state.type === manager || props.prop.location.state.type === manager&&
                <Fragment>
                  <TableCell>
                    <ApproveRejectButton type="submit" variant="contained" full={false} label="view" color="primary" />
                  </TableCell>
                  <TableCell onClick={() => removeRow(row.id)}>
                    <ApproveRejectButton type="submit" variant="contained" full={false} label="Approve" color="secondary" />
                  </TableCell>
                </Fragment>
              }
            </TableRow>
